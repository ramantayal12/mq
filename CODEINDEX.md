...
| Path | Responsibility |
|------|----------------|
| [cmd/mqbroker/main.go](cmd/mqbroker/main.go) | Entrypoint: load config, build broker + coordinator + server, run flush/retention loops, handle SIGINT/SIGTERM. Starts the Raft controller when the cluster has >1 broker **or** the object backend is selected (the object index lives in the FSM, so a single node auto-bootstraps a 1-server quorum); `buildObjectStorage` wires a minio-go `ObjectStore` + per-node WAL into `object.Storage` indexed through `controller.IndexStore()`, installed via `b.SetObjectStorage` before `SetController`. The `healthcheck` subcommand ([cmd/mqbroker/healthcheck.go](cmd/mqbroker/healthcheck.go), intercepted at the top of `main` before `config.Load`) performs a Kafka ApiVersions handshake against a running broker and exits 0/1 — the in-binary probe wired as the Docker `HEALTHCHECK` since the distroless image has no shell/curl. |
| [internal/config/config.go](internal/config/config.go) | `Config` + `Load()` — flags > `KAFKA_*` env > defaults (incl. `NodeID`, `Brokers`). `StorageBackend` (`KAFKA_STORAGE_BACKEND`/`--storage-backend`, `local`\|`object`) selects the partition store; `KAFKA_OBJECT_*` (endpoint/bucket/access-key/secret-key/region/upload-bytes/upload-ms) configure the object backend (connection params are env-only). |
